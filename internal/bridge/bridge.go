...
func Prepare(stateDir string, enabled bool, helperArch string) (*Session, error) {
	...
	if err := fileutil.WriteFileAtomic(helperPath, []byte(openShim(session)), fileutil.WriteOptions{Mode: 0o755, DirMode: 0o755}); err != nil {
		return nil, err
	}
	if err := fileutil.WriteFileAtomic(filepath.Join(binPath, "xdg-open"), []byte(xdgOpenShim()), fileutil.WriteOptions{Mode: 0o755, DirMode: 0o755}); err != nil {
		return nil, err
	}
...
func installHelperBinary(binPath string, helperArch string) error {
	...
	if err := fileutil.WriteFileAtomic(helperPath, data, fileutil.WriteOptions{Mode: 0o755, DirMode: 0o755}); err != nil {
		return err
	}
