...
- repo-local `.hatchctl/config.toml` values for `workspace`, `state_dir`, `cache_dir`, `bridge`, `ssh`, and `dotfiles` are ignored unless the user explicitly opts in with `--trust-workspace` or `HATCHCTL_TRUST_WORKSPACE=1`
